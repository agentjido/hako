defmodule Jido.VFS.Visibility.PortableUnixVisibilityConverter do
  @moduledoc """
  `Jido.VFS.Visibility.UnixVisibilityConverter` supporting `Jido.VFS.Visibility.portable()`.
  ...
  def for_file(%Config{} = config, visibility) do
    with {:ok, visibility} <- Jido.VFS.Visibility.guard_portable(visibility) do
      case visibility do
