  ...
  describe "polymorphic versioning API" do
    ...
    test "ETS adapter works with main Hako API" do
      ...
      assert {:ok, "versioned content"} =
               Jido.VFS.read_revision(filesystem, "test.txt", revision.revision)

      # Test rollback
      assert :ok = Jido.VFS.rollback(filesystem, revision.revision, path: "test.txt")
    end
    ...
    test "unsupported adapters return proper errors" do
      ...
      assert {:error, :unsupported} = Jido.VFS.commit(filesystem, "test")
      assert {:error, :unsupported} = Jido.VFS.revisions(filesystem, "test.txt")
      assert {:error, :unsupported} = Jido.VFS.read_revision(filesystem, "test.txt", "rev")
      assert {:error, :unsupported} = Jido.VFS.rollback(filesystem, "rev")
    end
